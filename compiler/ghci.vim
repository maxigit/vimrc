...
" XXX FIXME - need to unbreak _i and _b that had bitrotted/never worked on " uni
x? -- gaal
...
map _e :call Browse("")<CR>1G:call Module()<CR>1GwgUl"bp:call NoBanner()<cr>k$:c
all Format()<CR>
map _I 0/[A-Z]/<CR>:noh<CR>"bye:call Browse(@b)<CR>:call New()<CR>"bp:call NoBan
ner()<cr>
...
if has("menu")
  ...
  amenu Haskell(GHCi).AddTypeDecl<tab>{visual}_t  :call AddTypeDecl(expand("<cwo
rd>"))<cr>
  amenu Haskell(GHCi).-Browse-                    :
  amenu Haskell(GHCi).BrowseCurrent<tab>_B        _B
  amenu Haskell(GHCi).BrowseImport<tab>_I         _I
  amenu Haskell(GHCi).ShowType                    :call ShowType(expand("<cword>
"))<cr>
  amenu Haskell(GHCi).ShowInfo                    :call ShowInfo(expand("<cword>
"))<cr>
  ...
  function! DeclarationsMenu()
    ...
    while output != ""
      ...
      let menuitem = "amenu Declarations.".ident.' :call LocalFind("'.ident.'")<
cr>'
...
function! Type(cmd,thing,postcmd)
  let output=system(a:cmd.escape(" ".a:thing,s:esc)." | ".g:haskell.expand("%").
a:postcmd)
  let output=substitute(output,"[^>]*> ","","")
  let @t = output
  return output
...
function! Find(what)
  if a:what != ""
    ...
    if match(output,"defined at") != -1
      let location=substitute(output,'^.* defined at \(\S\+\):\(\d\+\)\D','+\2 \
1','')
      echo location
